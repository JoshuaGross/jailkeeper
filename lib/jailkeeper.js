...
module.exports = function (options) {
  ...
  var trussRespond = function (out) {
    ...
    for (var i in lines) {
      ...
      if (file || mode) {
        ...
        console.log('breaking the jailz:', mode, file);
        if ('read' === mode) {
          fs.stat(file, function (deetz) {
            return deetz && jailbreak(mode, file, line);
          });
        } else {
  ...
  var spawnJail = function (pid, cmd, args) {
    ...
    jail.childProcess.on('exit', function (code) {
      console.log('received child process exit')
      jail.childProcess = null;
    ...
    jailProcess.on('exit', function (code) {
      //activePIDs[pid] = null;
      jails--;
      console.log('jails left', jails)
      if (0 === jails) {
        console.log('send exit signal')
        jail.emit('exit');
        if (jail.childProcess) {
  ...
  jail.spawn = function (cmd, args, options) {
    ...
    jail.childProcess.stdout.on('data', function (d) {
      //console.log('child process sez', d.toString());
