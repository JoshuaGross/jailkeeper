...
module.exports = function (options) {
  ...
  jail.spawn = function (cmd, args) {
    var operatingSystem = require('os').platform();
    var truss = ('solaris' === operatingSystem ? 'truss' : ('darwin' === operatingSystem ? 'dtruss' : 'truss'));

    jail.childProcess = spawn(cmd, args, { cwd: options.cwd });
    jail.childProcess.stderr.on('data', dbg);
    jail.childProcess.stdout.on('data', dbg);
